...
6. TXT는 UTF-8이어야 하며, 생성 시 metadata hash를 신뢰하지 않고 읽은
   snapshot bytes에서 SHA-256을 새로 계산해 manifest에 기록한다.
...
- 낡은 metadata hash가 있어도 snapshot bytes의 실제 SHA-256으로 생성됨
