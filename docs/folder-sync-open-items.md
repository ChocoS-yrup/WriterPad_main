...
**원격 삭제된 폴더의 분리 문서 복원 — 해결됨**

원격 폴더 삭제를 Windows가 받을 때 문서 tombstone이 먼저 적용되므로 자식 문서는
휴지통 루트에, 빈 폴더는 별도 휴지통 항목에 보존된다. 예전의 사용자 주도 폴더 복원은
폴더만 live로 게시하고 분리된 자식 문서 행은 tombstone으로 남겼다. iPad에서는 복원된
로컬 문서와 뒤늦은 tombstone 보관본이 겹치면서 `이름__문서UUID.txt`가 휴지통에 남을
수 있었다.

이제 Windows에서 그 폴더를 복원하면 휴지통 색인의 원위치가 해당 폴더 하위인 문서를
찾되, 색인의 `document_id`/원위치, identity UUID, SQLite tombstone UUID/서버 경로가
모두 일치하는 항목만 함께 복원한다. 목적지가 이미 차 있거나 UUID·경로 증명이
불완전하거나 문서 작업이 대기 중이면 전체 재결합을 보류한다. 복원 게시 순서는
원본 폴더 UUID live, 원본 문서 UUID live, tree-order 순서로 유지된다.
...
## `get_sync_handshake` 응답 봉투 — 원본은 iPad 저장소다

```
ChocoS-yrup/Writerpad   sync-contract/handshake-envelope.md
```

`get_sync_handshake` 가 돌려주는 열 개 키와, 두 클라이언트가 무엇을 필수로 보는지
적혀 있다. 2026-08-25 까지 그 봉투는 **배포된 함수 몸통에만** 존재했다 — 어느
저장소에도 어느 명세에도 없었고, 스테이징을 내렸으면 같이 사라졌다.

**여기에 사본을 두지 않는다.** 한 번 미러로 넣었다가 뺐다. 이유는 이렇다.

사본을 두면 원본이 움직였을 때 조용히 낡는데, 그걸 잡을 방법이 마땅치 않다.
잡으려면 원본을 조회해야 하고, 그건 네트워크를 읽는 일이라 이 저장소의 밀폐
시험에 넣을 수 없다. CI 단계로 만들려면 워크플로를 고쳐야 하는데 그건 금지돼
있다. **미러 하나를 위해 그 둘 중 하나를 푸는 것보다, 갈릴 수 있는 사본을 두지
않는 편이 싸다.**

잃는 것은 오프라인에서 못 읽는 것뿐이다. 두 저장소가 같은 계정·같은 호스트에
있어서, 원본이 사라지는 경우라면 이 저장소도 같이 사라진다.

**Windows 가 행동에 쓰는 사실은 여기 있다.** 위 "핸드셰이크 응답의 선택적 세 키"
항목이 그것이고, 그 갈림은 이 저장소의 성질이므로 이 저장소가 들고 있는 것이
맞다. 봉투의 나머지는 원본을 보면 된다.

계약 pin 과는 무관하다. 다이제스트는 `protocol.json` 한 파일의 RFC 8785 바이트에만
걸린다.
...
**전환은 검증06·07 이 끝난 뒤 마지막 걸음이다 — 2026-08-25 결정.**

근거는 아래 "옮긴 뒤 다시 세워야 하는 것" 과 같다. **canary 기준선은 새
인스턴스로 이월되지 않는다.** 지금 옮기면 iPad 가 연결해서 대조할 대상이
사라지고, 계약 경로에서 관측한 다섯 가지가 근거를 잃는다.

그리고 새 인스턴스가 나은 점이 없다. 계약 0.2.0 범위에서 스테이징의 부분집합이며
(0.3.0 계열만 빠져 있다), 옮겨서 얻는 기능이 없다. 계정도 작품도 데이터도 없어서
연결 상태를 처음부터 다시 세워야 한다.

이 항목을 적어 두는 이유는 "지금 옮기자" 가 한 번 나왔기 때문이다. 다시 나올 때
붙잡으라고 남긴다.
