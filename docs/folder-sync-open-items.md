...
더해진 파일은 CI 에 딸려오지 않는다(그 커밋에는 이 파일이 아예 없다 — 404).
미러가 필요한 이유가 그것이고, 검사가 `contract_root()` 를 쓰면 안 되는 이유이기도
하다. 그 헬퍼는 CI 에서 고정 사본을 가리킨다.

### 규율을 강제하는 것

산문으로만 있는 규율은 규율이 아니다 — 이 저장소에서 여덟 번 나온 그 모양이다.
실패가 두 종류이고, 하나는 막았고 하나는 아직이다.

| 실패 | 막는 것 |
|---|---|
| **이 사본을 직접 고친다** | `tests/test_handshake_envelope_mirror.py`. 기록된 blob id 와 대조하고 어긋나면 실패한다. 네트워크 없이 812 안에서 돈다 |
| **원본이 바뀌었는데 미러를 안 한다** | **아직 없다.** 네트워크와 비교할 ref 가 필요하다 |

둘째를 미룬 이유는 이 문서가 아직 원본 저장소의 기본 브랜치에 없기 때문이다
(`fix/server-contract-migrations` 에 있다). 움직이는 기능 브랜치와 대조하는 검사는
의미가 없다. **기본 브랜치에 올라오면 그때 넣는다.** 미러가 하나 더 늘기 전에
하는 편이 값이 크다.

갱신은 한 커밋에서 한다 — 원본을 받아 덮고, 시험의 `SOURCE_COMMIT` 과
`SOURCE_BLOB_ID` 를 같이 고친다. GitHub contents API 의 `.sha` 가 곧
`git hash-object` 값이라 복제 없이 대조된다.
