...
class IndependentProjectBackupTests(unittest.TestCase):
    ...
    def test_creation_replaces_stale_metadata_hash_with_snapshot_hash(self):
        wrong_entries = self._entries(chapter_hash="0" * 64)
        before = self._regular_files(self.workspace)
        package = self.container / "스냅샷-해시-백업"
        ...
        self.assertNotEqual(chapter["content"]["sha256"], "0" * 64)
        self.assertEqual(self.store.verify_backup(package), receipt.manifest)
